...
export default function GeoPage() {
  ...
  const goToCheck = useCallback((raw: string) => {
    router.push(`/neyroseti?url=${encodeURIComponent(raw)}`);
  }, [router]);
  ...
  return (
    <div className="mrc-root">
      ...
      <main>
        ...
        <div className="mrc-wrap">
          {/* ─── 07 · Разбираем частые обещания ─── */}
          <section className="mrc-sec" data-reveal>
            <SecHead
              idx="07"
              title="Разбираем частые обещания"
              sub="Фразы, которые чаще всего слышат от подрядчиков в этой нише. Ни одна из них невыполнима — показываем, почему."
            />
