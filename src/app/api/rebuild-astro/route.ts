...
export interface OptIssue {
  ...
  fixed: boolean;
  /** true — клиент одобрил, но реального авто-фикса нет (risk:"manual") — передано команде на ручную доработку. */
  queued?: boolean;
  /** Машинный ключ проблемы — по нему этап оптимизации помечает fixed. */
  key?: string;
  /**
   * Только для "ручных" пунктов (без key в старом смысле) — можно ли их
   * одобрить клиентом через /api/rebuild-astro/optimize-item:
   * "safe" — механическая правка, без риска (объединение CSS, минификация HTML);
   * "moderate" — меняет поведение загрузки скриптов (defer), очень редко может
   *   сломать скрипт, зависящий от синхронного порядка исполнения;
   * "manual" — реального безопасного авто-фикса нет (например, удаление jQuery
   *   рискует сломать скрипты сайта) — одобрение ставит задачу на ручную доработку,
...
export function detectPerf(html: string, origin: string, techStack: string[]): OptimizationReport {
  ...
  if (techStack.includes("jQuery")) {
    issues.push({
      key: "jquery", risk: "manual", severity: "info",
      title: "Используется jQuery",
      detail: "Устаревшая библиотека (~90 КБ). Оставлена, потому что на ней держатся скрипты исходного сайта — автоматически убрать её небезопасно, можно сломать слайдеры или формы. Одобрение здесь не запускает автоматику, а ставит задачу на ручную доработку нашей команде.",
      fixed: false,
    });
  }
  if (extCss > 8) {
