...
1. hookText — крючок для ПЕРВЫХ 2-3 секунд, крупный текст на экране. Короткий (до 60 знаков), цепляющий, на языке сценария. НЕ повторяй дословно первую фразу озвучки — это отдельный визуальный крючок, он может быть острее/короче.
   ...
   НО: «острее» — это про формулировку, а НЕ про добавление новых фактов.
   В hookText, ctaText и keyPoints запрещено вводить то, чего нет в сценарии:
   - сроки и длительности («за 60 секунд», «за 3 дня», «под ключ за месяц»);
   - числа, проценты, количества, цены, скидки;
   - гарантии и обещания («гарантируем», «дешевле всех», «лучшие в городе»);
   - факты о компании, которых нет во входных данных.
   Реальный случай, который так и вышел в эфир: сценарий был про репортаж со
   стройплощадки, а хук получился «От чертежа к реальности за 60 секунд» —
   срок выдуман, да ещё и противоречил хронометражу ролика.
   Если хочется цифру, а её нет в сценарии — переформулируй без цифры.
...
3. brollQueries — 3-4 английские короткие фразы (2-5 слов каждая) — сцены, которыми AI-видеогенератор проиллюстрирует ролик. Конкретные, предметные (например "dentist examining patient", "musician recording session"), НЕ абстрактные ("business success").
   ...
   СВЯЗЬ С ТЕКСТОМ: каждая сцена должна показывать то, О ЧЁМ РЕЧЬ в этот момент озвучки, а не «что-то из той же отрасли». Если в сценарии говорится про подготовку основания — проси именно бетонный пол, а не общий вид стройки. Зритель сверяет картинку со словами, и расхождение читается как «взяли первый попавшийся сток».

   ЛЮДИ В КАДРЕ — ПОЛНЫЙ ЗАПРЕТ: не заказывай сцены с людьми ВООБЩЕ. Ни рабочих, ни рук, ни силуэтов, ни «человек на фоне». Только материалы, конструкции, оборудование, интерьеры, фактуры, панорамы.

   Это не перестраховка, а вывод из двух провалов подряд. Сначала просили «естественную анатомию» — получили рабочего, висящего в воздухе у металлокаркаса. Потом запретили высоту и лазание — следующая генерация выдала человека на балках и человека БЕЗ ГОЛОВЫ, которому кровельная панель прошла сквозь шею. Модель не умеет связывать тело с конструкцией и режет анатомию за предметами переднего плана.

   Вместо «worker welding steel beam» проси «close-up of welded steel joint, sparks cooling, macro»; вместо «engineer inspecting site» — «steel frame structure against sky, slow camera pan». Живой человек в ролике и так есть — это ведущий во врезке, он синтезируется другим движком, который на людях специализируется.
...
4. keyPoints — 2 КОРОТКИХ тезиса (3-6 слов каждый) для текстовых карточек между видеофрагментами. Это не пересказ, а опорные фразы: то, что зритель должен запомнить. Берутся по смыслу из озвучки, но формулируются коротко — длинное предложение на карточке занимает пол-экрана и не читается за 3 секунды. Пример: вместо «Три подрядчика назвали три разные цены за один и тот же объект» — «Три цены за один объект».
5. mood — настроение фоновой музыки, СТРОГО одно из: "upbeat" (энергичное, продающее), "calm" (спокойное, доверительное), "corporate" (нейтрально-деловое), "dramatic" (напряжённое, для проблема→решение), "playful" (лёгкое, с юмором).

Отвечай СТРОГО валидным JSON без markdown, БЕЗ пояснений:
{"hookText":"...","ctaText":"...","brollQueries":["...","...","..."],"keyPoints":["...","..."],"mood":"corporate"}`;
...
Об аватаре: avatar.placement — говорящая голова (реалистичный видео-аватар), озвученная ТЕМ ЖЕ голосом, что и весь ролик, вставленная в наш кадр. "off" — без аватара, это разумный выбор по умолчанию. "pip" — круглая врезка в углу поверх видеоряда: экспертное объяснение, разбор, личный бренд, «говорит основатель», ролик-ответ на вопрос. "full" — аватар занимает целый сегмент видеоряда вместо иллюстрации: прямое обращение, приглашение, анонс. Ставь "off", когда ролик про предмет, интерьер, процесс, атмосферу или продукт — говорящая голова там перетягивает внимание с того, что показываем. Не ставь аватара «для разнообразия»: он уместен только там, где содержание — это чьё-то личное высказывание.
...
interface PlanResult { hookText: string; ctaText: string; brollQueries: string[]; keyPoints?: string[]; mood?: string }
