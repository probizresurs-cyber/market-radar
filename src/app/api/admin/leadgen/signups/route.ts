/**
 * Прокси из админки MarketRadar к /api/admin/signups лидгена (заявки с лендинга).
...
function cfg() {
  ...
  return { url: `${base}/api/admin/signups`, token };
}
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    const r = await fetch(`${url}?id=${encodeURIComponent(id)}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } });
    return NextResponse.json(await r.json().catch(() => ({})), { status: r.status });
  } catch (e) {
    return NextResponse.json({ error: `Лидген недоступен: ${(e as Error).message}` }, { status: 502 });
  }
