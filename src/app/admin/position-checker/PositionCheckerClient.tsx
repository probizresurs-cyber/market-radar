  ...
  return (
    <div style={S.page}>
      ...
      <main style={S.main}>
        ...
        <div style={S.sub}>
          Живая проверка реальной позиции — не оценка от AI. Yandex проверяется через официальный
          Yandex Search API (без браузера, без риска капчи). Google — по-прежнему headless-браузером,
          последовательно с паузами между словами, на 10 слов может уйти 1.5-2 минуты; если Google
          показал капчу или заблокировал запрос — строка будет помечена «не удалось проверить», позиция
          не выдумывается. Осторожный лимит: не больше 3 проверок в день.
        </div>
