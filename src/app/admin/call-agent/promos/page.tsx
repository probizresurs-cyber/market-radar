...
export default function CAPromosPage() {
  ...
  async function load() {
    ...
    setErr(null);
    try {
      const r = await fetch("/api/admin/call-agent/promos", { cache: "no-store" });
      const d = await r.json().catch(() => ({}));
      if (!r.ok || !d.ok) {
        setErr(d.error || `HTTP ${r.status}`);
      } else {
        setRows(Array.isArray(d.promos) ? d.promos : []);
      }
    } catch (e) {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const r = await fetch("/api/admin/call-agent/promos", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          bonus_calls: bonusRef.current?.value ? Number(bonusRef.current.value) : 0,
      ...
      if (!r.ok || !d.ok) {
        ...
        if (bonusRef.current) bonusRef.current.value = "";
  ...
  return (
    <main style={S.main}>
      <div style={S.h1}>Промокоды Call-Agent</div>
      <div style={S.sub}>Промокоды со скидками и бонусными звонками для клиентов Call-Agent.</div>
      ...
      {showForm && (
        <div style={S.card}>
          <div style={S.cardTitle}>Новый промокод</div>
          <form onSubmit={handleCreate}>
            <div style={S.formGrid}>
              <div>
                <label style={S.label}>Код *</label>
                <input ref={codeRef} style={S.input} placeholder="SALE30" required />
              </div>
              <div>
                <label style={S.label}>Описание</label>
                <input ref={descRef} style={S.input} placeholder="Акция лето 2025" />
              </div>
              <div>
                <label style={S.label}>Скидка %</label>
                <input ref={discountRef} style={S.input} type="number" min="0" max="100" placeholder="30" />
              </div>
              <div>
                <label style={S.label}>Бонус звонков</label>
                <input ref={bonusRef} style={S.input} type="number" min="0" placeholder="50" />
              </div>
              <div>
      ...
      <div style={S.card}>
        {loading ? (
          ...
          <div style={S.emptyMsg}>Промокодов пока нет. Создайте первый.</div>
        ) : (
            ...
            <table style={S.table}>
              <thead>
                <tr>
                  ...
                  <th style={S.th}>Бонус звонков</th>
              ...
              <tbody>
                {rows.map((p, i) => (
                  <tr key={p.id} style={{ background: i % 2 === 0 ? "#131720" : "#0f1117" }}>
                    <td style={{ ...S.td, color: "#475569", fontSize: 11 }}>{p.id}</td>
                    <td style={{ ...S.td, ...S.mono }}>{p.code}</td>
                    <td style={{ ...S.td, color: "#94a3b8" }}>{p.description || "—"}</td>
                    <td style={{ ...S.td, color: p.discount_pct > 0 ? "#4ade80" : "#64748b", fontWeight: p.discount_pct > 0 ? 700 : 400 }}>
                      {p.discount_pct > 0 ? `${p.discount_pct}%` : "—"}
                    </td>
                    <td style={{ ...S.td, color: p.bonus_calls > 0 ? "#f59e0b" : "#64748b", fontWeight: p.bonus_calls > 0 ? 700 : 400 }}>
                      {p.bonus_calls > 0 ? `+${p.bonus_calls}` : "—"}
                    </td>
                    <td style={{ ...S.td, color: "#e2e8f0", fontWeight: 600 }}>{p.uses_count}</td>
                    <td style={{ ...S.td, color: "#64748b" }}>{p.max_uses ?? "∞"}</td>
                    <td style={S.td}>
                      {p.active
                        ? <span style={S.badge("#4ade80")}>Да</span>
                        : <span style={S.badge("#64748b")}>Нет</span>}
                    </td>
                    <td style={{ ...S.td, color: "#64748b", fontSize: 12 }}>{fmtDate(p.expires_at)}</td>
