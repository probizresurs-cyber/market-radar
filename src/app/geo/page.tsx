...
export default function GeoPage() {
   ...
   * проверки при желании сможет подставить его в поле (сейчас параметр не
   * читает, см. TODO ниже).
   *
   * TODO(бэкенд): контакты из формы отправляются в теле запроса, но
   * `POST /api/mini-check` читает только `url` и молча их отбрасывает —
   * email и телефон с /geo нигде не сохраняются. Нужно либо доучить роут
   * писать контакты в `mini_checks`, либо после получения `id` дёргать
   * `POST /api/mini-check/lead {id, email, consent}` (он уже умеет и лида
   * сохранить, и КП поставить). До этого форма работает как «стартер
   * проверки», а не как захват лида.
   */
