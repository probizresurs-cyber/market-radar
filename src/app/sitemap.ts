...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const staticEntries: MetadataRoute.Sitemap = [
    ...
    // именно на них идут кампании: /neyroseti — основная посадочная
    // (канонический адрес воронки; /check — тот же экран, /new — редирект
    // сюда для уже запущенных объявлений и /нейросети для набравших
    // адрес по-русски), /geo — кластер про нейросети,
    // /competitors — кластер про анализ конкурентов.
    // Все три — короткие мини-лендинги. Длинные версии (/new2, /geo2,
    // /competitors2) в карту НЕ входят: они закрыты noindex, потому что
    // рассказывают то же самое и конкурировали бы с короткими за те же запросы.
    {
