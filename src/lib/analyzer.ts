...
export async function analyzeWithClaude(data: ScrapedData, businessType?: BusinessType, locale: "ru" | "de" = "ru"): Promise<AnalysisResult & { _usage?: { inputTokens: number; outputTokens: number } }> {
  ...
  // По умолчанию — российский рынок (основной продукт платформы, все существующие
  // вызовы без locale работают как раньше). "de" — только для авто-КП /kp-de
  // (kp-generate.ts), где сайт немецкий и весь текстовый ответ должен быть на
  // немецком — иначе, например, aiPerception.sampleAnswer выходит на русском
  // для немецкой компании (найдено на живом тесте).
  const marketLine = locale === "de"
    ? "Ты эксперт по цифровому маркетингу, SEO и конкурентному анализу для немецкого рынка. ВСЕ текстовые поля в ответе — на НЕМЕЦКОМ языке."
...
Если данных не хватает — НЕ выдумывай, ставь null или "${locale === "de" ? "unzureichende Daten" : "недостаточно данных"}" в соответствующих полях (на ${locale === "de" ? "немецком" : "русском"}).
${businessTypeHint}
...
- Все текстовые поля — ${locale === "de" ? "на немецком (Deutsch), включая aiPerception.sampleAnswer" : "на русском"}.
