...
const PRICE_POLICY: Record<KpLocale, {
  ...
  ru: {
    ...
    ours: "от 25 000 ₽/мес",
    // Разовый первый шаг — 25 000 ₽. Раньше стояло 10 000 ₽, и это
    // расходилось с посадочной, где оптимизация первого месяца объявлена
    // за 25 000 ₽. Человек читал одну цену на лендинге и другую в разборе;
    // после такого он не верит и остальным числам в документе.
    astro: "25 000 ₽",
...
- deeperScope — то, что мы видим, но не успели разобрать в этом документе.
  Смысл главы: это ЛЕСТНИЦА. Собранное в документе — то, что видно снаружи за
  один проход. Дальше идёт глубокий разбор, и только после него — само
  продвижение. Поэтому среди пунктов ОБЯЗАТЕЛЬНО должен быть слой SEO/GEO:
  полная семантика и кластеры (документ смотрел только верхушку), страницы под
  каждый кластер, разметка услуг и цен, видимость в ассистентах по всем целевым
  вопросам клиента — а не только по тем, что попали в этот разбор. Остальные
  пункты — из этого набора: аналитика ЦА, путь клиента, отзывы и репутация,
  ассортимент и офферы, воронка внутри сайта, конкурентная разведка. Пиши
  конкретно по ЭТОМУ бизнесу, без общих слов: 4-5 пунктов, каждый — что именно
...
export async function generateKp(rawUrl: string, locale: KpLocale): Promise<KpGenResult> {
  ...
  try {
    ...
    // Поиск каналов вне сайта (lib/brand-socials.ts) из конвейера убран:
    // он требует Yandex Search API, а платный ключ пока не заводим. Модуль
    // остался — включить обратно можно одной строкой, когда ключ появится.
    // Скорость сборки для КП важнее, чем догадки о каналах.
