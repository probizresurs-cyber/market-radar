...
export interface CreatePayload {
  ...
  engine?: { type: string };
  /**
   * Интенсивность мимики: "high" | "medium" | "low". По документации HeyGen
   * работает ТОЛЬКО для фото-аватаров и ТОЛЬКО на движке Avatar IV, а по
   * умолчанию стоит "low" — то есть наши аватары всё это время говорили с
   * минимальной мимикой просто потому, что поле не передавалось.
   * С engine=avatar_v поле отвергается.
   */
  expressiveness?: string;
  /**
   * Описание движений и жестов словами. Для фото-аватаров работает на обоих
   * движках, для видео-аватаров — только на avatar_v.
   */
