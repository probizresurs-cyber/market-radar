...
function ruBlocks(seoGeoPrice: string): Blocks {
  return {
    ...
    market: {
      // Цена подставляется В СЕРЕДИНУ фразы: строки прайса начинаются со
      // строчного «от», и в начале предложения это читается как опечатка.
      intro:
        `Наш тариф — ${seoGeoPrice}, и это ниже вилки, в которой работают GEO-агентства. За такие деньги рынок обычно продаёт «тариф-галочку»: одно-два действия в месяц и отчёт о проделанной работе. Поэтому разницу показываем в цифрах, а не на словах.`,
      rows: [
