...
/** Telegram-alert — тот же inline-паттерн, что в ai-visibility-monitor.ts. */
async function sendTelegram(userId: string, text: string): Promise<void> {
  ...
  try {
    ...
    console.warn("[geo-agent] TG send failed:", e);
  }
}
...
registerAgent({
  ...
  async run(ctx: AgentContext): Promise<AgentRunResult> {
    ...
    if (alertText) await sendTelegram(ctx.userId, alertText);
