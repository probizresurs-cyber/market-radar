...
/**
 * Фактура для генератора: включённые модули платформы (таблица features) +
...
export async function productContext(): Promise<string> {
  ...
  try {
    await initDb();
    const rows = await query<{ label: string; description: string | null }>(
      `SELECT label, description FROM features
       WHERE enabled = true AND parent_id IS NULL
  ...
  return [
    ...
    `контент-маркетинга и бренд-стратегии. Работает на Claude (Anthropic).`,
    ``,
    `Включённые модули платформы (только про них можно писать как про существующие):`,
    modules || "- (список модулей недоступен — пиши только о платформе в целом)",
    ``,
    `Цены (менять нельзя):`,
