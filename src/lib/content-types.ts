...
export interface BrandBook {
  ...
  /**
   * Банк проверенных фактов — ЕДИНСТВЕННЫЙ источник цифр для генерации.
   *
   * Появился после инцидента: презентация ОРЛИНК ушла с выдуманными
   * «3 завода» и «с 2014 года», а лендинг — с «500+ объектов». Модель
   * охотно сочиняет правдоподобные цифры о чужой компании, и показать
   * такое клиенту — провал доверия. Все generate-роуты обязаны брать
   * конкретику ТОЛЬКО отсюда; пусто — плейсхолдер, а не выдумка.
   */
  facts?: BrandFacts;
  /**
   * Фотобанк: URL реальных снимков компании (/api/static-asset/brand-photos/…).
   * Файлы лежат на сервере — в брендбуке только ссылки, иначе десяток фото
   * в base64 раздул бы user_data и localStorage на мегабайты.
   * Реальные фото поднимают качество сильнее любого промпта: презентации
   * ставят их на слайды, лендинги — в галерею, ролики — в видеоряд вместо
   * платной AI-генерации.
   */
  photos?: string[];
}

/** Проверенные факты компании. Заполняет человек, модель их не трогает. */
export interface BrandFacts {
  /** Год основания, например «2014» */
  foundedYear?: string;
  /** Реализованные объекты/проекты, например «120+ объектов» */
  completedProjects?: string;
  /** Команда/мощности, например «3 цеха, 85 сотрудников» */
  capacity?: string;
  /** География работы */
  geography?: string;
  /** Известные клиенты (через запятую) */
  clients?: string;
  /** Сертификаты, СРО, допуски */
  certifications?: string;
  /** Реальные кейсы с цифрами — свободный текст */
  cases?: string;
  /** Прочие проверенные цифры и факты — свободный текст */
  extra?: string;
}
