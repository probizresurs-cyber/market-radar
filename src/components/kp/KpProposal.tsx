...
// Цены здесь обязаны совпадать с PRICE_POLICY в src/lib/kp-generate.ts и с
// тем, что напечатано на лендингах (/new, /geo, /competitors): человек читает
// «от 25 000 ₽/мес» на посадочной, и вторая цифра в документе обнуляет доверие
// ко всем остальным. Эта таблица показывается только вне pilot-ветки
// (/share/[id], /kp) — рекламная воронка идёт через PRICE_POLICY.
const PACKAGES = [
  {
    ...
    tiers: [
      { tier: "Старт", price: "от 25 000 ₽/мес", items: ["Технический аудит + правки", "Семантика по 1 кластеру", "Оптимизация 5 страниц"] },
  ...
  },
  {
    ...
    accent: "var(--success)",
    tiers: [
      { tier: "Старт", price: "от 25 000 ₽/мес", items: ["8 постов в соцсети", "Контент-план на месяц", "Ведение СММ"] },
