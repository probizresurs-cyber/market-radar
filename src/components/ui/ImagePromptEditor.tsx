...
export interface ImagePromptEditorProps {
  ...
  /** Заголовок панели. По умолчанию «Промпт для DALL-E». */
  title?: string;
...
export function ImagePromptEditor({ params, generateLabel = "Сгенерировать фото", title = "Промпт для DALL-E", onGenerate, onCancel }: ImagePromptEditorProps) {
  ...
  const fetchSuggestion = async () => {
    ...
    try {
      ...
      const j = await r.json() as { ok: boolean; prompt?: string; error?: string };
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 13, color: "var(--foreground-secondary)", lineHeight: 1.5 }}>
        Claude предложил промпт ниже. <strong>Можно использовать как есть, отредактировать
        или попросить новый вариант.</strong> На английском — DALL-E лучше понимает.
      </div>
        ...
        {suggesting && !prompt && (
          <div style={{
            ...
            <span style={{ display: "inline-flex", alignItems: "center", gap: 8, fontSize: 13, color: "var(--muted-foreground)" }}>
              <Loader2 size={15} style={{ animation: "spin 1s linear infinite" }} /> Claude пишет промпт…
            </span>
          </div>
        )}
        <textarea
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
        ...
        >
          {generating
            ? <><Loader2 size={15} style={{ animation: "spin 1s linear infinite" }}/> Рисую через DALL-E…</>
        ...
        <button
          ...
          disabled={disabled}
          title="Claude напишет другой вариант промпта"
          style={{
            padding: "11px 16px",
            borderRadius: 10,
            border: "1.5px solid var(--border)",
