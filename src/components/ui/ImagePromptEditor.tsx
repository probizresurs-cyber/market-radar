...
interface SuggestParams {
  ...
  brandColors?: string[];
  brandStyle?: string;
  // Контекст компании — без него AI промахивается на омонимах (стоматология
  // для стройки и т.п.). Опциональные поля для обратной совместимости.
  companyName?: string;
  companyNiche?: string;
...
export interface ImagePromptEditorProps {
  ...
  title?: string;
  /** Если задан — показываем выбор формата (квадрат / портрет / альбом)
   *  и при смене вызываем callback. Caller должен подкрутить body /api/generate-image. */
  onFormatChange?: (format: "пост" | "сторис" | "пост-горизонтальный") => void;
  /** Если задан — показываем выбор платформы (instagram/vk/telegram).
   *  Влияет на стиль (узкий формат для TG, квадрат для VK и т.п.). */
  onPlatformChange?: (platform: "instagram" | "vk" | "telegram" | "tiktok") => void;
...
export function ImagePromptEditor({ params, generateLabel = "Сгенерировать фото", title = "Промпт для AI-генератора", onFormatChange, onPlatformChange, onGenerate, onCancel }: ImagePromptEditorProps) {
  const [prompt, setPrompt] = useState("");
  const [suggesting, setSuggesting] = useState(true);
  const [generating, setGenerating] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [selectedFormat, setSelectedFormat] = useState<"пост" | "сторис" | "пост-горизонтальный">(
    (params.format === "сторис" ? "сторис" : params.format === "пост-горизонтальный" ? "пост-горизонтальный" : "пост")
  );
  const [selectedPlatform, setSelectedPlatform] = useState<"instagram" | "vk" | "telegram" | "tiktok">(
    (params.platform as "instagram" | "vk" | "telegram" | "tiktok") || "instagram"
  );
  ...
  const paramsKey = JSON.stringify({
    ...
    companyNiche: params.companyNiche ?? "",
  ...
  return (
    <div style={{
      ...
      <div style={{ position: "relative" }}>
        ...
        {/* Селекторы формата + платформы — если caller прокинул callbacks */}
        {(onFormatChange || onPlatformChange) && (
          <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginTop: 10, padding: "10px 12px", background: "var(--background)", borderRadius: 8, border: "1px solid var(--border)" }}>
            {onFormatChange && (
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                <span style={{ fontSize: 10, fontWeight: 700, color: "var(--muted-foreground)", textTransform: "uppercase", letterSpacing: "0.05em" }}>Формат</span>
                <div style={{ display: "flex", gap: 4 }}>
                  {([
                    { id: "пост" as const, label: "□ Квадрат" },
                    { id: "сторис" as const, label: "▮ 9:16" },
                    { id: "пост-горизонтальный" as const, label: "▭ 16:9" },
                  ]).map(f => (
                    <button key={f.id} type="button"
                      onClick={() => { setSelectedFormat(f.id); onFormatChange(f.id); }}
                      style={{
                        padding: "5px 10px", borderRadius: 6, fontSize: 11, fontWeight: 700,
                        border: `1px solid ${selectedFormat === f.id ? "var(--primary)" : "var(--border)"}`,
                        background: selectedFormat === f.id ? "color-mix(in oklch, var(--primary) 12%, transparent)" : "transparent",
                        color: selectedFormat === f.id ? "var(--primary)" : "var(--foreground-secondary)",
            ...
            {onPlatformChange && (
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                <span style={{ fontSize: 10, fontWeight: 700, color: "var(--muted-foreground)", textTransform: "uppercase", letterSpacing: "0.05em" }}>Платформа</span>
                <div style={{ display: "flex", gap: 4 }}>
                  {([
                    { id: "instagram" as const, label: "Insta" },
                    { id: "vk" as const, label: "VK" },
                    { id: "telegram" as const, label: "TG" },
                    { id: "tiktok" as const, label: "TikTok" },
                  ]).map(p => (
                    <button key={p.id} type="button"
                      onClick={() => { setSelectedPlatform(p.id); onPlatformChange(p.id); }}
                      style={{
                        padding: "5px 10px", borderRadius: 6, fontSize: 11, fontWeight: 700,
                        border: `1px solid ${selectedPlatform === p.id ? "var(--primary)" : "var(--border)"}`,
                        background: selectedPlatform === p.id ? "color-mix(in oklch, var(--primary) 12%, transparent)" : "transparent",
                        color: selectedPlatform === p.id ? "var(--primary)" : "var(--foreground-secondary)",
                        cursor: "pointer", fontFamily: "inherit",
                      }}>{p.label}</button>
                  ))}
                </div>
