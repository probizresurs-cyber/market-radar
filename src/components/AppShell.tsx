...
// ============================================================
// Main App — shell, scoped по продукту (core / seo-geo / content-factory / land-pres)
// ============================================================

export default function MarketRadarDashboard({ scope = "core" }: { scope?: ProductScope }) {
  return (
    <ToastProvider>
      <MarketRadarDashboardInner scope={scope} />
    </ToastProvider>
  );
}

function MarketRadarDashboardInner({ scope }: { scope: ProductScope }) {
  ...
  const [activeNav, setActiveNav] = useState(defaultNavForScope(scope));
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const router = useRouter();
  // Если на платформу пришли с ?nav=<id> (например, с дашборда руководителя), переключаемся на эту секцию.
  // Если таб принадлежит другому продукту — уводим на его маршрут.
  useEffect(() => {
    if (typeof window === "undefined") return;
    try {
      ...
      if (!navParam) return;
      const targetScope = scopeForNav(navParam);
      if (targetScope !== scope) {
        router.replace(`${PRODUCT_BY_SCOPE[targetScope].route}?nav=${navParam}`);
        return;
      }
  ...
  // «Разбор ролика» (Тренды) уже отдаёт готовый GeneratedReel из /adapt —
  // просто кладём его в библиотеку, без повторной генерации.
  const handleSendReelToLibrary = React.useCallback((reel: GeneratedReel) => {
    setGeneratedReels(prev => {
    ...
    toast({
      ...
      description: "Добавлен в «Готовые рилсы» — можно сгенерировать видео.",
  ...
  // Нав-секции, отфильтрованные под текущий продукт (scope). На / (core) —
  // Обзор+Аналитика+Настройки; на продуктовых маршрутах — только их секции.
  const navSections = scopedNavSections(scope).map(section => ({ ...section, items: updateCounts(section.items) }));
