...
export function InsightsView({ c, data, competitors }: { c: Colors; data: AnalysisResult; competitors: AnalysisResult[] }) {
  ...
  return (
    <div style={{ maxWidth: 860 }}>
      ...
      {/* ── Инсайты ── */}
      {/* Тип "battle" фильтруем — battle-cards теперь живут в отдельном модуле
          «Конкуренты → Battle cards», тут они дублировали бы. */}
      <div style={{ display: "flex", flexDirection: "column", gap: 14, marginBottom: 28 }}>
        {data.insights.filter(ins => ins.type !== "battle").map((ins, i) => {
      ...
      {/* Battle Cards перенесены в отдельный модуль «Конкуренты → Battle cards» —
          здесь их повторно не показываем, чтобы не дублировать UI. */}
