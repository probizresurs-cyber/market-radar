...
export function ContentAnalyticsView({ c, posts, reels, companyName }: {
  ...
  if (allRows.length === 0) {
    return (
      <div style={{ maxWidth: 800 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, margin: "0 0 8px", color: "var(--foreground)" }}>📊 Аналитика контента</h1>
  ...
  const KpiCell = ({ label, value, sub, color }: { label: string; value: string; sub?: string; color?: string }) => (
    <div style={{ flex: "1 1 140px", padding: "12px 14px", background: "var(--card)", borderRadius: 12, border: `1px solid var(--border)`, boxShadow: "var(--shadow)" }}>
      <div style={{ fontSize: 10, fontWeight: 700, color: "var(--muted-foreground)", letterSpacing: "0.05em", marginBottom: 4 }}>{label}</div>
      <div style={{ fontSize: 22, fontWeight: 800, color: color ?? "var(--foreground)", lineHeight: 1.1 }}>{value}</div>
  ...
  return (
    <div style={{ maxWidth: 1200 }}>
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, margin: "0 0 4px", color: "var(--foreground)" }}>📊 Аналитика контента</h1>
        <p style={{ fontSize: 13, color: "var(--muted-foreground)", margin: 0 }}>{filteredRows.length} публикаций с метриками · {allRows.length - filteredRows.length} вне фильтра</p>
      </div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
        {([[0, "Всё время"], [7, "7 дней"], [30, "30 дней"], [90, "90 дней"]] as const).map(([days, label]) => (
          <button key={days} onClick={() => setPeriodDays(days)}
            style={{ padding: "6px 14px", borderRadius: 8, border: `1.5px solid ${periodDays === days ? "#f59e0b" : "var(--border)"}`,
              background: periodDays === days ? "#f59e0b15" : "var(--background)", color: periodDays === days ? "#f59e0b" : "var(--foreground-secondary)",
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 18 }}>
        ...
        <KpiCell label="ROMI" value={totalROMI != null ? `${totalROMI.toFixed(0)}%` : "—"} color={totalROMI != null ? (totalROMI >= 0 ? "#22c55e" : "var(--destructive)") : undefined} />
      </div>
      ...
      <Card style={{ marginBottom: 18 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, flexWrap: "wrap", gap: 10 }}>
          ...
          <button
            ...
            style={{ padding: "9px 18px", borderRadius: 9, border: "none", background: loadingInsights ? "var(--muted)" : "linear-gradient(135deg, #6366f1, #8b5cf6)",
              color: loadingInsights ? "var(--muted-foreground)" : "#fff", fontSize: 12, fontWeight: 800, cursor: loadingInsights ? "not-allowed" : "pointer",
        ...
        {insightsError && <div style={{ background: "color-mix(in oklch, var(--destructive) 7%, transparent)", color: "var(--destructive)", padding: "8px 12px", borderRadius: 8, fontSize: 11, marginBottom: 10 }}>❌ {insightsError}</div>}
        {insights && <InsightsDisplay c={c} insights={insights} />}
      </Card>

      {/* Sort selector */}
      <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 10, flexWrap: "wrap" }}>
        <span style={{ fontSize: 11, fontWeight: 700, color: "var(--muted-foreground)", letterSpacing: "0.05em" }}>СОРТИРОВКА:</span>
        {([["er", "ER"], ["reach", "Охват"], ["romi", "ROMI"], ["revenue", "Выручка"], ["date", "Дата"]] as const).map(([k, label]) => (
          <button key={k} onClick={() => setSortBy(k)}
            style={{ padding: "5px 11px", borderRadius: 7, border: `1px solid ${sortBy === k ? "var(--primary)" : "var(--border)"}`,
              background: sortBy === k ? "color-mix(in oklch, var(--primary) 8%, transparent)" : "transparent", color: sortBy === k ? "var(--primary)" : "var(--foreground-secondary)",
      ...
      <Card style={{ padding: 0, overflow: "hidden" }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
            <thead>
              <tr style={{ background: "var(--background)", borderBottom: `1px solid var(--muted)` }}>
                {["Тип", "Заголовок", "Пиллар", "Формат", "Охват", "ER", "Лиды", "Выручка", "ROMI"].map(h => (
                  <th key={h} style={{ textAlign: "left", padding: "10px 12px", fontSize: 10, fontWeight: 700, color: "var(--muted-foreground)", letterSpacing: "0.05em" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {sortedRows.map((row, i) => {
                const erColor = row.er >= avgER * 1.3 ? "#22c55e" : row.er < avgER * 0.7 ? "var(--destructive)" : "var(--foreground-secondary)";
                const romiColor = row.romi == null ? "var(--muted-foreground)" : row.romi >= 100 ? "#22c55e" : row.romi >= 0 ? "var(--foreground-secondary)" : "var(--destructive)";
                return (
                  <tr key={row.id} style={{ borderBottom: i < sortedRows.length - 1 ? `1px solid var(--muted)` : "none" }}>
                    ...
                    <td style={{ padding: "9px 12px", color: "var(--foreground)", maxWidth: 280, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{row.title}</td>
                    <td style={{ padding: "9px 12px", color: "var(--foreground-secondary)" }}>{row.pillar}</td>
                    <td style={{ padding: "9px 12px", color: "var(--muted-foreground)", fontSize: 10 }}>{row.format}</td>
                    <td style={{ padding: "9px 12px", fontWeight: 700, color: "var(--foreground)", fontFamily: "monospace" }}>{fmtNumber(row.reach)}</td>
                    <td style={{ padding: "9px 12px", fontWeight: 800, color: erColor, fontFamily: "monospace" }}>{row.er.toFixed(1)}%</td>
                    <td style={{ padding: "9px 12px", color: "var(--foreground-secondary)", fontFamily: "monospace" }}>{row.leads || "—"}</td>
                    <td style={{ padding: "9px 12px", color: "var(--foreground-secondary)", fontFamily: "monospace" }}>{row.revenue > 0 ? fmtNumber(row.revenue) + " ₽" : "—"}</td>
...
export function InsightsDisplay({ c, insights }: { c: Colors; insights: PerformanceInsights }) {
  ...
  return (
    <div>
      <div style={{ padding: "10px 14px", background: "#6366f10c", borderRadius: 8, border: "1px solid #6366f125", fontSize: 12, fontWeight: 600, color: "var(--foreground)", lineHeight: 1.55 }}>
        💡 {insights.summary}
      </div>
        ...
        <Section title="🏆 ТОП КОНТЕНТ-СТОЛПЫ">
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 10 }}>
            {insights.topPillars.map((p, i) => (
              <div key={i} style={{ padding: 12, background: "var(--background)", borderRadius: 8, border: `1px solid var(--muted)` }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "var(--foreground)", marginBottom: 4 }}>{p.name}</div>
                <div style={{ fontSize: 11, color: "var(--foreground-secondary)", marginBottom: 6 }}>
                  ER <b style={{ color: "#22c55e" }}>{p.avgER?.toFixed?.(1) ?? p.avgER}%</b> · охват <b>{fmtNumber(p.totalReach)}</b>
                </div>
                <div style={{ fontSize: 11, color: "var(--muted-foreground)", lineHeight: 1.5 }}>{p.why}</div>
              </div>
            ))}
          </div>
        </Section>
        ...
        <Section title="📐 ТОП ФОРМАТЫ">
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {insights.topFormats.map((f, i) => (
              <div key={i} style={{ padding: "8px 12px", background: "var(--background)", borderRadius: 8, border: `1px solid var(--muted)` }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: "var(--foreground)" }}>{f.format} <span style={{ color: "#22c55e" }}>{f.avgER?.toFixed?.(1) ?? f.avgER}%</span></div>
                <div style={{ fontSize: 10, color: "var(--muted-foreground)", marginTop: 2 }}>{f.why}</div>
              </div>
            ))}
          </div>
        </Section>
      ...
      {insights.topHooks?.length > 0 && (
        <Section title="🪝 РАБОЧИЕ КРЮЧКИ">
          {insights.topHooks.map((h, i) => (
            <div key={i} style={{ padding: 10, background: "var(--background)", borderRadius: 8, border: `1px solid var(--muted)`, marginBottom: 6 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: "var(--foreground)", marginBottom: 4 }}>{h.pattern}</div>
              {h.examples?.length > 0 && (
                <div style={{ fontSize: 10, color: "var(--muted-foreground)", marginBottom: 4, fontStyle: "italic" }}>
                  {h.examples.map(e => `«${e}»`).join(" · ")}
                </div>
              )}
              <div style={{ fontSize: 11, color: "var(--foreground-secondary)" }}>{h.why}</div>
            </div>
          ))}
        </Section>
      )}
      ...
      {insights.underperformers?.length > 0 && (
        <Section title="⚠️ ЧТО НЕ РАБОТАЕТ">
          {insights.underperformers.map((u, i) => (
            <div key={i} style={{ padding: 10, background: "color-mix(in oklch, var(--destructive) 3%, transparent)", borderRadius: 8, border: `1px solid var(--destructive)25`, marginBottom: 6 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: "var(--destructive)", marginBottom: 3 }}>{u.what}</div>
              <div style={{ fontSize: 10, color: "var(--muted-foreground)", marginBottom: 3 }}>метрика: {u.metric}</div>
              <div style={{ fontSize: 11, color: "var(--foreground-secondary)" }}>👉 {u.fix}</div>
            </div>
          ))}
        </Section>
      )}
        ...
        <Section title="🎯 РЕКОМЕНДАЦИИ">
          <ol style={{ paddingLeft: 18, margin: 0 }}>
            {insights.recommendations.map((r, i) => (
              <li key={i} style={{ fontSize: 12, color: "var(--foreground-secondary)", lineHeight: 1.6, marginBottom: 4 }}>{r}</li>
            ))}
          </ol>
        </Section>
      )}
        ...
        <Section title="📅 ПЛАН НА СЛЕДУЮЩУЮ НЕДЕЛЮ">
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 6 }}>
            {insights.nextWeekPlan.map((d, i) => (
              <div key={i} style={{ padding: "8px 10px", background: "var(--background)", borderRadius: 7, border: `1px solid var(--muted)`, fontSize: 11, color: "var(--foreground-secondary)" }}>{d}</div>
            ))}
          </div>
        </Section>
      )}
...
export function ROICalculatorView({ c, posts, reels }: {
  ...
  const Card = ({ children, style }: { children: React.ReactNode; style?: React.CSSProperties }) => (
    <div style={{ background: "var(--card)", borderRadius: 14, border: `1px solid var(--border)`, padding: 18, boxShadow: "var(--shadow)", ...style }}>{children}</div>
  );

  const labelStyle: React.CSSProperties = { display: "block", fontSize: 10, fontWeight: 700, color: "var(--muted-foreground)", marginBottom: 5, letterSpacing: "0.05em" };
  ...
  return (
    <div style={{ maxWidth: 1100 }}>
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, margin: "0 0 4px", color: "var(--foreground)" }}>💰 ROI калькулятор</h1>
      ...
      <div style={{ marginBottom: 18 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: "var(--muted-foreground)", letterSpacing: "0.05em", marginBottom: 8 }}>📈 ВАШИ ИСТОРИЧЕСКИЕ ПОКАЗАТЕЛИ</div>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          {[
            ...
            { label: "ROMI ИСТОРИЧ.", value: historicalROMI != null ? `${historicalROMI.toFixed(0)}%` : "—",
              color: historicalROMI != null ? (historicalROMI >= 0 ? "#22c55e" : "var(--destructive)") : undefined },
          ].map((k, i) => (
            <div key={i} style={{ flex: "1 1 160px", padding: "12px 14px", background: "var(--card)", borderRadius: 12, border: `1px solid var(--border)`, boxShadow: "var(--shadow)" }}>
              <div style={{ fontSize: 9, fontWeight: 700, color: "var(--muted-foreground)", letterSpacing: "0.05em", marginBottom: 4 }}>{k.label}</div>
              <div style={{ fontSize: 20, fontWeight: 800, color: k.color ?? "var(--foreground)" }}>{k.value}</div>
              {k.sub && <div style={{ fontSize: 10, color: "var(--muted-foreground)", marginTop: 3 }}>{k.sub}</div>}
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        <Card>
          <div style={{ fontSize: 13, fontWeight: 800, color: "var(--foreground)", marginBottom: 14 }}>📥 Параметры на месяц</div>
          ...
          {avgCPL === 0 && (
            <div style={{ marginTop: 12, padding: "8px 12px", background: "#f59e0b15", borderRadius: 7, fontSize: 11, color: "var(--foreground-secondary)" }}>
              ⚠️ Внесите метрики хотя бы по 3-5 публикациям с указанием рекламного бюджета и лидов — иначе прогноз будет нулевым.
            </div>
          )}
        </Card>

        <Card style={{ background: `linear-gradient(135deg, var(--card), #6366f108)` }}>
          <div style={{ fontSize: 13, fontWeight: 800, color: "var(--foreground)", marginBottom: 14 }}>🎯 Прогноз на месяц</div>
          <div style={{ display: "grid", gap: 12 }}>
            ...
            <ForecastRow c={c} label="Чистая прибыль" value={forecastProfit !== 0 ? `${fmtNumber(Math.round(forecastProfit))} ₽` : "—"}
              color={forecastProfit > 0 ? "#22c55e" : forecastProfit < 0 ? "var(--destructive)" : undefined}
              sub={`${marginPct}% маржи − реклама`} />
            <ForecastRow c={c} label="ROMI" value={monthlyBudget > 0 ? `${forecastROMI.toFixed(0)}%` : "—"}
              color={forecastROMI > 0 ? "#22c55e" : forecastROMI < 0 ? "var(--destructive)" : undefined} />
            <div style={{ height: 1, background: "var(--muted)", margin: "4px 0" }} />
      ...
      <Card style={{ marginTop: 16 }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: "var(--foreground)", marginBottom: 6 }}>💡 Как читать прогноз</div>
        <ul style={{ margin: 0, paddingLeft: 18, fontSize: 12, color: "var(--foreground-secondary)", lineHeight: 1.7 }}>
...
export function ForecastRow({ c, label, value, sub, color }: { c: Colors; label: string; value: string; sub?: string; color?: string }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0" }}>
      <div>
        <div style={{ fontSize: 11, color: "var(--foreground-secondary)", fontWeight: 600 }}>{label}</div>
        {sub && <div style={{ fontSize: 10, color: "var(--muted-foreground)", marginTop: 2 }}>{sub}</div>}
      </div>
      <div style={{ fontSize: 18, fontWeight: 800, color: color ?? "var(--foreground)", fontFamily: "monospace" }}>{value}</div>
    </div>
  );
}
