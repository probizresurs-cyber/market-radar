...
export function PresentationView({ c, myCompany, taAnalysis, smmAnalysis, brandBook, userId }: {
    ...
    try {
      const res = await fetch("/api/presentation-share", {
        ...
        body: JSON.stringify({
          ...
          // Лого и имя бренда — чтобы публичная страница рисовала фирменную
          // марку так же, как кабинет (иначе клиент видел слайды без логотипа).
          logoUrl: brandBook.logoDataUrl,
          brandName: brandBook.brandName || myCompany?.company?.name,
        }),
      });
  ...
  // Разметка слайда вынесена в SlideCanvas — тот же компонент рисует
  // публичную страницу /pres/[slug], чтобы кабинет и ссылка для клиента
  // никогда не расходились по виду (раньше это были два разных рендера).
  const renderSlide = (slide: PresentationSlide, idx: number) => (
    <SlideCanvas
      ...
      primary={primary}
      secondary={secondary}
      fontHeader={sty.fontHeader || brandBook.fontHeader || "Georgia"}
      fontBody={sty.fontBody || brandBook.fontBody || "Inter"}
      logoUrl={brandBook.logoDataUrl}
      brandName={brandBook.brandName || myCompany?.company?.name}
    />
  );
