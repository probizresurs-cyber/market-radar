#!/bin/bash
# Периодические задачи market-radar: дожим лида, ТГ-прогрев, контент-лента канала.
#
...
run "lead-followups"
run "kp-tg-warm"
# Публикует одобренные отложенные посты, по расписанию заводит авто-черновик
# (пн/ср/пт) и раз в сутки обновляет описание продукта для промпта бота —
# см. src/lib/channel-poster.ts. Все три действия сами решают, пора ли им
# что-то делать, так что вызывать каждый час безопасно.
run "channel-posts"
